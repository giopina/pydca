...
class DCA:
    ...
    def __comp_true_freq(self):
        ...
        if self.theta > 0.0 :
            ...
            W= (1./(1+_np.sum(squareform(cacca),axis=0)))
        self.Meff=_np.sum(W)
    ...
    def __to_ising_gauge(self):
        ...
        J_avg=_np.average(self.__invC,axis=3)
        ### Possa dio aver pieta' di noi
        self.__invC+=_np.average(J_avg,axis=1)[:,_np.newaxis,:,_np.newaxis]\
                      -J_avg[:,:,:,_np.newaxis]\
    ...
    def __comp_CFN(self,no_gaps=False):
        ...
        F_sum=_np.sum(self.CFN,axis=0)
        self.CFN-=F_sum[:,_np.newaxis]*F_sum[_np.newaxis,:]/_np.sum(F_sum)
